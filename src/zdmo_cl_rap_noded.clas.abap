CLASS zdmo_cl_rap_noded DEFINITION
  PUBLIC
  CREATE PUBLIC
*  GLOBAL FRIENDS ZDMO_cl_rap_xco_json_visitor
  ...
  PUBLIC SECTION.
    ...
    TYPES :  tt_childnodes TYPE STANDARD TABLE OF REF TO zdmo_cl_rap_noded WITH EMPTY KEY.
    TYPES :  ty_childnode TYPE REF TO zdmo_cl_rap_noded.
    ...
    DATA all_childnodes TYPE STANDARD TABLE OF REF TO zdmo_cl_rap_noded READ-ONLY.
    DATA childnodes TYPE STANDARD TABLE OF REF TO zdmo_cl_rap_noded READ-ONLY.
    DATA root_node TYPE REF TO zdmo_cl_rap_noded READ-ONLY.
    DATA parent_node TYPE REF TO zdmo_cl_rap_noded READ-ONLY.
    ...
    METHODS constructor
      ...
      RAISING   ZDMO_cx_rap_generatord.
    ...
    METHODS set_ext_element_Suffix
      IMPORTING VALUE(iv_ext_element_Suffix) TYPE string
      RAISING   ZDMO_cx_rap_generatord.
    ...
    METHODS set_parent
      IMPORTING
                io_parent_node TYPE REF TO zdmo_cl_rap_noded
      RAISING   ZDMO_cx_rap_generatord.

    METHODS set_root
      IMPORTING
                io_root_node TYPE REF TO zdmo_cl_rap_noded
      RAISING   ZDMO_cx_rap_generatord.
...
*        it_additional_fields TYPE tt_additional_fields.
  ...
  PRIVATE SECTION.
    ...
    METHODS add_to_all_childnodes
      IMPORTING VALUE(io_child_node) TYPE REF TO zdmo_cl_rap_noded.

    METHODS add_child_node_hierarchy
      IMPORTING
                child_node TYPE REF TO zdmo_cl_rap_noded
      RAISING   ZDMO_cx_rap_generatord.
...
CLASS zdmo_cl_rap_noded IMPLEMENTATION.
...
*    ENDLOOP.
  ...
  METHOD add_additional_fields_old.
    ...
    CASE to_lower( iv_object ).
      ...
      WHEN OTHERS.
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid   = ZDMO_cx_rap_generatord=>cannot_add_additional_fields
            mv_value = iv_object.
    ENDCASE.
  ...
  METHOD add_association.
    ...
    IF underscore <> '_'.
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid   = ZDMO_cx_rap_generatord=>starts_not_with_underscore
    ...
    ELSE.
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid   = ZDMO_cx_rap_generatord=>assocation_is_not_supported
  ...
  METHOD add_child.
    ...
    DATA lt_all_childnodes  TYPE STANDARD TABLE OF REF TO zdmo_cl_rap_noded .

    DATA   ls_childnode  TYPE REF TO zdmo_cl_rap_noded  .

    ro_child_node = NEW zdmo_cl_rap_noded( ).
    ...
    IF me->is_extensible(  ) = abap_true.
      ro_child_node->set_ext_element_suffix( me->extensibility_element_suffix ).
    ENDIF.
      ...
      LOOP AT it_additional_binding INTO DATA(ls_additional_binding).
        ...
        CASE ls_additional_binding-usage .
          ...
          WHEN OTHERS.
            RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
              EXPORTING
                textid   = ZDMO_cx_rap_generatord=>usage_is_not_supported
...
*          <field_quan>-is_hidden = abap_true.
  ...
  METHOD add_virtual_root_node.
    ...
    ro_virtual_root_node = NEW zdmo_cl_rap_noded( ).
  ...
  METHOD check_data_elements_of_fields.
    ...
    "@todo:
    "add package1 as an attribute to the class zdmo_cl_rap_noded so that it does not have to be calculated
    "every time
...
**            abap_language_version_number = zdmo_cl_rap_noded=>package_abap_language_version-abap_for_sap_cloud_platform.
*
**          IF data_element->content( )->has_underlying_built_in_type( ).
*
*            DATA(underlying_built_in_type) = data_element->content( )->get_underlying_built_in_type( ).
...
*        ENDIF.
  ...
  METHOD check_parameter.
    ...
    "  RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
    "    EXPORTING
    "      textid            = ZDMO_cx_rap_generatord=>parameter_is_initial
    ...
    IF contains_no_blanks( CONV #( iv_value ) ) = abap_false.
      ...
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid   = ZDMO_cx_rap_generatord=>contains_spaces
    ...
    IF is_alpha_numeric( CONV #( iv_value ) ) = abap_false.
      ...
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid   = ZDMO_cx_rap_generatord=>non_alpha_numeric_characters
          mv_value = |Object:{ iv_parameter_name } Name:{ iv_value }|.
    ENDIF.
    ...
    IF strlen( iv_value ) > lv_max_length.
      ...
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid        = ZDMO_cx_rap_generatord=>is_too_long
...
*        ENDIF.
  ...
  METHOD check_repository_object_name.
    ...
    IF lv_object_already_exists = abap_true.
      ...
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid     = ZDMO_cx_rap_generatord=>repository_already_exists
          mv_value   = lv_name
          mv_value_2 = lv_type.
    ENDIF.
    ...
    IF ( lv_type = 'TABL(Database Table)' OR lv_type = 'TABL(Structure)' ) AND
      ...
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid   = ZDMO_cx_rap_generatord=>underscore_not_at_pos_2_3
          mv_value = lv_name.
    ENDIF.
  ...
  METHOD check_table_package_assignment.
    ...
    IF swc_name_package <> swc_name_db_table.
      IF NOT swc_name_db_table = 'ZDMO_SAP'  AND  swc_name_db_table = 'ZLOCAL'.
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid          = ZDMO_cx_rap_generatord=>software_comp_do_not_match
...
*    binding_type = binding_type_name-odata_v4_ui.
  ...
  METHOD finalize.
    ...
    IF namespace IS INITIAL.
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid = ZDMO_cx_rap_generatord=>no_namespace_set.
    ENDIF.
    ...
    CASE data_source_type.
      WHEN data_source_types-table.
        IF table_name IS INITIAL.
          RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
            EXPORTING
              textid    = ZDMO_cx_rap_generatord=>no_data_source_set
    ...
    IF implementationtype = implementation_type-unmanaged_semantic OR
      ...
      IF semantic_key IS INITIAL .
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid = ZDMO_cx_rap_generatord=>no_semantic_key_set.
      ENDIF.
    ENDIF.
    ...
    IF object_id IS INITIAL.
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid    = ZDMO_cx_rap_generatord=>no_object_id_set
          mv_entity = entityname.
    ENDIF.
    ...
    IF lt_fields IS INITIAL.
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid    = ZDMO_cx_rap_generatord=>no_fields_found
          mv_entity = entityname.
    ENDIF.
...
*    ENDIF.
  ...
  METHOD generate_bil.
    ...
    IF get_implementation_type(  )  = zdmo_cl_rap_noded=>implementation_type-managed_uuid.
      DATA(is_managed_uuid) = abap_true.
    ENDIF.

    "all nodes that use managed_managed need a determination for the field object_id
    IF get_implementation_type(  )  = zdmo_cl_rap_noded=>implementation_type-managed_semantic.
    ...
    IF get_implementation_type(  )  = zdmo_cl_rap_noded=>implementation_type-unmanaged_semantic .
...
*'VARC'  Obsolete Data Type
  ...
  METHOD get_abap_type_components.
    ...
    ELSE.
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid     = ZDMO_cx_rap_generatord=>invalid_abap_type_name
...
*        set_mapping(  ).
      ...
      WHEN OTHERS.
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid   = ZDMO_cx_rap_generatord=>invalid_data_source_type
...
*            DATA(rtti_problem_view_root) = cl_message_helper=>get_latest_t100_exception( cx_rtti_problem_view )->if_message~get_longtext( ).
      ...
      WHEN OTHERS.
        ...
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid    = ZDMO_cx_rap_generatord=>view_type_not_supported
...
*      DATA(lv_name) = |{ namespace }I_{ prefix }{ entityname }{ suffix }|.
    ...
    ELSE.
      ...
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid    = ZDMO_cx_rap_generatord=>is_not_a_root_node
...
*      ENDCASE.
      ...
      ELSEIF lo_domain->exists(  ) = abap_true AND
             abap_language_version_number = zdmo_cl_rap_noded=>package_abap_language_version-standard.
...
*    ENDIF.
  ...
  METHOD set_binding_type.
    ...
    ELSE.
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid     = ZDMO_cx_rap_generatord=>invalid_binding_type
  ...
  METHOD set_cds_view.
    ...
    IF  lt_result IS INITIAL .
      IF xco_lib->get_data_definition( CONV #(  lv_cds_view )  )->exists( ) = abap_false .
        ...
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid   = ZDMO_cx_rap_generatord=>cds_view_does_not_exist
...
*        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
*          EXPORTING
*            textid   = ZDMO_cx_rap_generatord=>table_is_inactive
    ...
    IF me->root_node IS NOT INITIAL.
      ...
      LOOP AT lt_all_childnodes INTO DATA(ls_childnode).
        IF ls_childnode->entityname = iv_entity_name.
          RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
            EXPORTING
              textid    = ZDMO_cx_rap_generatord=>entity_name_is_not_unique
  ...
  METHOD set_implementation_type.
    ...
    CASE iv_implementation_type.
      ...
      WHEN OTHERS.
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid   = ZDMO_cx_rap_generatord=>implementation_type_not_valid
      ...
      IF it_field_mappings IS NOT INITIAL.
        ...
        LOOP AT it_field_mappings INTO DATA(ls_field_mapping) WHERE dbtable_field  <> field_name-client.
          ...
          ELSE.
            RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
              EXPORTING
                textid        = ZDMO_cx_rap_generatord=>field_is_not_in_datasource
    ...
    ELSEIF ( data_source_type = data_source_types-cds_view OR data_source_type = data_source_types-abstract_entity ) AND
      ...
      LOOP AT it_field_mappings INTO ls_field_mapping.
        ...
        IF field_name_exists_in_cds_view( CONV #( ls_field_mapping-cds_view_field ) ) = abap_false.
          RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
            EXPORTING
              textid    = ZDMO_cx_rap_generatord=>field_is_not_in_cds_view
  ...
  METHOD set_namespace.
    ...
    IF iv_namespace IS NOT INITIAL.
      ...
      ELSE.
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid   = ZDMO_cx_rap_generatord=>invalid_namespace
...
*      DATA(lv_name) = |{ namespace }{ prefix }{ entityname }{ suffix }|.
  ...
  METHOD set_package.
    ...
    IF NOT xco_lib->get_package( iv_package )->exists(  ) OR iv_package IS INITIAL.
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid   = ZDMO_cx_rap_generatord=>package_does_not_exist
    ...
    ELSE.
      ...
      IF data_source_type = data_source_types-table  OR
        ...
        IF xco_lib->get_database_table(  CONV #( lv_table ) )->exists( ) = abap_false.
          ...
          RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
            EXPORTING
              textid   = ZDMO_cx_rap_generatord=>table_does_not_exist
              mv_value = CONV #( lv_table ).
        ENDIF.
...
*        data(table_delivery_class) = xco_lib->get_database_table( CONV #( lv_table ) )->content(  )->get(  )-delivery_class->value.
  ...
  METHOD set_transport_request.
    ...
    IF me->package IS NOT INITIAL.
      ...
      IF record_object_changes = abap_false AND iv_transport_request IS NOT INITIAL.
        ...
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid     = ZDMO_cx_rap_generatord=>invalid_transport_request
    ...
    IF iv_transport_request IS NOT INITIAL.
      ...
      IF transport_object->exists(  ) = abap_false.
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid   = ZDMO_cx_rap_generatord=>transport_does_not_exist
            mv_value = CONV #( iv_transport_request ).
      ENDIF.
  ...
  METHOD validate_bo.
    ...
    IF package_type <> zdmo_cl_rap_noded=>package_type-development_package.
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid          = ZDMO_cx_rap_generatord=>invalid_package_type
    ...
    IF data_source_type = data_source_types-table.
      ...
      IF result_client IS INITIAL.
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid   = ZDMO_cx_rap_generatord=>clnt_is_not_key_field
    ...
    IF generate_custom_entity(  ) AND draft_enabled = abap_true.
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid    = ZDMO_cx_rap_generatord=>no_draft_for_cust_entites
          mv_entity = entityname.
    ENDIF.
    ...
    IF ( is_child( ) OR is_grand_child_or_deeper(  ) ) AND
      ...
      IF NOT line_exists( lt_fields[ name = field_name-etag_master ] ).
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid    = ZDMO_cx_rap_generatord=>no_etag_delegation_support
            mv_entity = entityname.
      ENDIF.
    ENDIF.
    ...
    IF implementationtype = implementation_type-managed_uuid.
      ...
      IF result_uuid IS INITIAL.
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid   = ZDMO_cx_rap_generatord=>field_uuid_missing
      ...
      ELSEIF numberofrecords = 1 AND
        ...
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid   = ZDMO_cx_rap_generatord=>uuid_has_invalid_data_type
            mv_value = CONV #( field_name-uuid ).
      ENDIF.
      ...
      IF result_uuid IS INITIAL.
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid    = ZDMO_cx_rap_generatord=>has_no_key_field
            mv_entity = entityname.
      ENDIF.
      ...
      IF numberofrecords = 1 AND  result_uuid[ 1 ]-name <> field_name-uuid.
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid    = ZDMO_cx_rap_generatord=>uuid_is_not_key_field
            mv_entity = entityname
      ...
      IF numberofrecords > 1.
        ...
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid    = ZDMO_cx_rap_generatord=>more_than_one_key_field
            mv_entity = entityname
            mv_value  = key_fields.
      ENDIF.
      ...
      IF is_child( ) OR is_grand_child_or_deeper(  ).
        ...
        IF result_parent_uuid IS INITIAL.
          RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
            EXPORTING
              textid   = ZDMO_cx_rap_generatord=>field_parent_uuid_missing
      ...
      IF is_grand_child_or_deeper(  ).
        ...
        IF result_root_uuid IS INITIAL.
          RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
            EXPORTING
              textid   = ZDMO_cx_rap_generatord=>field_root_uuid_missing
    ...
    IF me->draft_enabled = abap_true AND me->draft_table_name IS INITIAL.
      ...
      IF draft_table_name IS INITIAL.
        ...
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid         = ZDMO_cx_rap_generatord=>no_draft_table_specified
    ...
    IF root_node->draft_enabled = abap_true.
      ...
      IF is_root(  ).
        ...
        ELSE.
          RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
            EXPORTING
              textid    = ZDMO_cx_rap_generatord=>field_total_etag_missing
              mv_entity = entityname.
        ENDIF.
...
*        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
*          EXPORTING
*            textid    = ZDMO_cx_rap_generatord=>field_local_etag_missing
...
*      ENDIF.
    ...
    ELSE.
      IF is_root(  ).
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid    = ZDMO_cx_rap_generatord=>field_etag_master_missing
            mv_entity = entityname.
      ELSE.
    ...
    LOOP AT lt_valuehelp INTO ls_valuehelp.
      ...
      IF table_name IS INITIAL.
        CASE   data_source_type.
          ...
          WHEN OTHERS.
            RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
              EXPORTING
                textid = ZDMO_cx_rap_generatord=>no_table_set.
        ENDCASE.
      ENDIF.
      ...
      IF NOT field_name_exists_in_cds_view( CONV #(  ls_valuehelp-localelement  ) ).
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid    = ZDMO_cx_rap_generatord=>not_a_field_in_cds_view
    ...
    IF implementationtype = implementation_type-unmanaged_semantic OR
      ...
      LOOP AT semantic_key ASSIGNING <ls_semantic_key>.
        ...
        IF resultsemantickeyalias IS INITIAL.
          RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
            EXPORTING
              textid                = ZDMO_cx_rap_generatord=>sematic_key_is_not_in_table
    ...
    IF me->manage_business_configuration = abap_true.
      ...
      IF me->draft_enabled = abap_false.
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid   = ZDMO_cx_rap_generatord=>not_draft_enabled
      ...
      IF me->binding_type <> binding_type_name-odata_v4_ui.
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid   = ZDMO_cx_rap_generatord=>no_ui_v4_service_binding
            mv_value = CONV #( me->root_node->entityname ).
      ENDIF.
    ...
    IF me->root_node->publish_service = abap_true
      ...
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid = ZDMO_cx_rap_generatord=>publish_needs_active_srvd.
    ...
    IF is_customizing_table = abap_true AND is_grand_child_or_deeper(  ).
      ...
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid     = ZDMO_cx_rap_generatord=>grand_child_not_supported
          mv_entity  = entityname
    ...
    IF multi_edit = abap_true AND
      ...
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid    = ZDMO_cx_rap_generatord=>mult_edit_not_supported
          mv_entity = entityname.
    ...
    IF is_customizing_table = abap_true.
      ...
      IF delivery_class <> 'C'.
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid     = ZDMO_cx_rap_generatord=>delivery_class_c_required
    ...
    IF bo_is_extensible = abap_true AND extensibility_element_suffix IS INITIAL.
      RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
        EXPORTING
          textid = ZDMO_cx_rap_generatord=>no_ext_element_suffix_set.
    ENDIF.
  ...
  METHOD set_ext_element_suffix.
    ...
    IF iv_ext_element_suffix IS NOT INITIAL.
      ...
      IF strlen( iv_ext_element_suffix ) NE 3.
        ...
        RAISE EXCEPTION TYPE ZDMO_cx_rap_generatord
          EXPORTING
            textid   = ZDMO_cx_rap_generatord=>invalid_ext_element_Suffix
